...
**This is the OG "hackable coding assistant" that [@neboysa](https://github.com/neboysa) and [@ierceg](https://github.com/ierceg) built back in 2016-2017 timeframe. We are preserving it here for historical and sentimental purposes. The new project you can find at https://gitlab.com/getlazy/ - see you there!**
...
* Run `make install` in the root of the cloned repository
* Run `make hack-<your option>` in the root of the cloned repository (e.g. `make hack-node-backend`)
...
* `node-backend`: runs NodeJS related engines

To configure your own engines see `lazy.yaml` section.
...
Engines are Docker containers running an HTTP server which follows a very simple API specification (see below)

### Lifecycle

During its lifecycle an engine will go through the following stages:

* clean: lazy checks if any engines are already running, stops them and removes their containers (this can happen if its root container was say stopped)
...
* `POST /file`: accepts the content of the file in the body (among other things), runs the linting process, returns results
