...
`
    ...
    then: (results) => {
        ...
        assert(warnings[0].message.endsWith('Unexpected var, use let or const instead.'));
        assert(warnings[1].message.endsWith('\'x\' is assigned a value but never used.'));
    },
    catch: ASSERT_FALSE
}, {
...
`
    ...
    then: (results) => {
        ...
        assert(warnings[0].message.endsWith('Parsing error: Unexpected token'));
    },
    catch: ASSERT_FALSE
}];
